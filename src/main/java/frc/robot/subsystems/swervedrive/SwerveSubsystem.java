...
public class SwerveSubsystem extends SubsystemBase {
  ...
  private double currentSpeed = Constants.MAX_SPEED;
  private final double maxAngularVelocity;

  private void configureDriveSettings() {
    swerveDrive.setHeadingCorrection(false); // Heading correction should only be used while controlling the robot via
                                             // angle.
    swerveDrive.setCosineCompensator(false); // Disable for simulation stability; the example keeps it off by default.
    swerveDrive.setAngularVelocityCompensation(true,
        true,
        0.1); // Correct for skew that gets worse as angular velocity increases. Start with a
              // coefficient of 0.1.
    swerveDrive.setModuleEncoderAutoSynchronize(false,
        1); // Enable to resynchronize encoders periodically when they are not moving.
  }
  ...
  public SwerveSubsystem(File directory) {
    ...
    try {
      swerveDrive = new SwerveParser(directory).createSwerveDrive(currentSpeed, startingPose);
    ...
    maxAngularVelocity = swerveDrive.getMaximumChassisAngularVelocity();
  ...
  public void changeSpeed(double newSpeed) {
    currentSpeed = MathUtil.clamp(newSpeed, Constants.MIN_SPEED, Constants.MAX_SPEED);
    swerveDrive.setMaximumAllowableSpeeds(currentSpeed, maxAngularVelocity);
  }

  public void adjustSpeed(double speedDelta) {
    changeSpeed(currentSpeed + speedDelta);
  }
